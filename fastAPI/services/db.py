...
import psycopg2
import redis
from .serial import read_and_parse_imu
    ...
    try:
        ...
        for frame in run_data.get("frames", []):
            try:
                ...
                # Insert frame without 'name' column (only run_id reference)
                cursor.execute(
...
def save_pid_settings(kp: float, ki: float, kd: float, speed: float):
    """
    ...
    try:
        ...
        cursor.execute(
            "INSERT INTO setting_pid (kp, ki, kd, speed) VALUES (%s, %s, %s, %s)",
            (kp, ki, kd, speed)
        )
        conn.commit()
        cursor.close()
        ...
        logger.info(f"Saved PID settings: kp={kp}, ki={ki}, kd={kd}, speed={speed}")
        return {
            "status": "ok",
            "message": "PID settings saved successfully"
        }

    except Exception as e:
        logger.error(f"Error saving PID settings: {e}")
        return {
            "status": "error",
            "message": str(e)
        }
...
def get_latest_pid_settings():
    """
    Get the latest PID settings from PostgreSQL.

    Returns:
        dict: Result with status, message, and PID settings
    """
    try:
        conn = psycopg2.connect(
            host=PG_HOST,
        ...
        cursor.execute(
            "SELECT id, kp, ki, kd, speed FROM setting_pid ORDER BY id DESC LIMIT 1"
        )
        row = cursor.fetchone()
        ...
        if not row:
            return {
                "status": "no_data",
                "message": "No PID settings found",
                "data": None
            }

        return {
            "status": "ok",
            "message": "Retrieved latest PID settings",
            "data": {
                "id": row[0],
                "kp": row[1],
                "ki": row[2],
        ...
        logger.error(f"Error retrieving PID settings: {e}")
        return {
            "status": "error",
            "message": str(e),
            "data": None
